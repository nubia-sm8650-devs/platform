...
#ifdef ZTE_FEATURE_PV_AR
	bdwlan_str = strnstr(fw_priv->fw_name, "bdwlan", strlen(fw_priv->fw_name));
	if (bdwlan_str != NULL) {
		size_t file_size = 0;
		size_t *file_size_ptr = NULL;

		len = snprintf(path, PATH_MAX, "%s%s%s",
					"/mnt/vendor/persist/", bdwlan_str, suffix);

		dev_warn(device, "bdwlan file path:%s", path);
