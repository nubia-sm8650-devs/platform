...
#define FP_DEV_VERSION "v0.3.9"

#define BSP_SIL_IRQ_ASYNC  /* IRQ use asynchrous mode. */

#ifdef ZTE_FINGERPRINT_PANEL_ENABLE
#ifndef ZTE_FEATURE_PV_AR
#define FP_PANEL_NOTIFY_PRIMARY
#ifdef ZTE_FEATURE_SECONDARY_ENABLE
#define FP_PANEL_NOTIFY_SECONDARY
#endif /* secondary enable*/
#endif /* PV */
#endif /* panel enable */
...
#if defined(FP_PANEL_NOTIFY_PRIMARY) || defined(FP_PANEL_NOTIFY_SECONDARY)
...
static inline void report_uevent(struct silfp_data *fp_dev, char *str)
{
	char *envp[2];

	envp[0] = str;
	envp[1] = NULL;
	kobject_uevent_env(&(fp_dev->spi->dev.kobj), KOBJ_CHANGE, envp);
}
...
static void silfp_netlink_send(struct silfp_data *fp_dev, const int cmd)
{
	...
	LOG_MSG_DEBUG(INFO_LOG, "silfp_netlink_send cmd %d\n", cmd);
...
static int silfp_keyevent(struct silfp_data	*fp_dev, struct fp_dev_key_t *pkey)
{
	...
	if (IS_KEY_VALID(pkey->value)) {
		...
		case NAV_KEY_CLICK_DOWN:
			...
			LOG_MSG_DEBUG(INFO_LOG, "[%s] fingerprint report down uevent\n", __func__);/*report down uevent*/
			report_uevent(fp_dev, FP_NAV_DOWN);
			break;
		case NAV_KEY_CLICK_UP:
			pkey->value = NAV_KEY_CLICK;
			pkey->flag = NAV_KEY_FLAG_UP;
			LOG_MSG_DEBUG(INFO_LOG, "[%s] fingerprint report up uevent\n", __func__);/*report up uevent*/
			report_uevent(fp_dev, FP_NAV_UP);
			break;
		default:
			break;
		}
...
#ifdef FP_PANEL_NOTIFY_PRIMARY
static struct drm_panel *silfp_active_panel = NULL;
static struct panel_event_notifier_entry *cookie = NULL;
static void silfp_drm_notifier_callback(enum panel_event_notifier_tag tag,
		struct panel_event_notification *notification, void *client_data);
#endif

#ifdef FP_PANEL_NOTIFY_SECONDARY
static struct drm_panel *silfp_active_panel_2nd = NULL;
static struct panel_event_notifier_entry *cookie_2nd = NULL;
static void silfp_drm_notifier_callback_2nd(enum panel_event_notifier_tag tag,
		struct panel_event_notification *notification, void *client_data);
#endif

#ifdef FP_PANEL_NOTIFY_PRIMARY
static int silfp_drm_get_pannel(struct device_node *np)
{
	int index = 0;
	int count = 0;
	...
	if (count <= 0) {
		LOG_MSG_DEBUG(ERR_LOG, "[%s] find panel node fail\n", __func__);
		return count;
	}
	...
	for (index = 0; index < count; index++) {
		...
		of_node_put(node);
		if (!IS_ERR(panel)) {
			LOG_MSG_DEBUG(ERR_LOG, "[%s] find panel success\n", __func__);
			silfp_active_panel = panel;
			LOG_MSG_DEBUG(ERR_LOG, "[%s] register drm notify, active =0x%x\n", __func__, silfp_active_panel);
			return 0;
		}
	}
	LOG_MSG_DEBUG(ERR_LOG, "[%s] find panel fail!\n", __func__);
	return count;
}
#endif

#ifdef FP_PANEL_NOTIFY_SECONDARY
static int silfp_drm_get_pannel_2nd(struct device_node *np)
{
	int index = 0;
	int count = 0;
	struct device_node *node = NULL;
	...
	count = of_count_phandle_with_args(np, "panel_2nd", NULL);
	if (count <= 0) {
		LOG_MSG_DEBUG(ERR_LOG, "[%s] find panel_2nd node fail\n", __func__);
		return -EINVAL;
	}

	for (index = 0; index < count; index++) {
		node = of_parse_phandle(np, "panel_2nd", index);
		panel = of_drm_find_panel(node);
		of_node_put(node);
		if (!IS_ERR(panel)) {
			LOG_MSG_DEBUG(ERR_LOG, "[%s] find panel_2nd success\n", __func__);
			silfp_active_panel_2nd = panel;
			LOG_MSG_DEBUG(ERR_LOG, "[%s] register drm notify, active_2nd =0x%x\n", __func__, silfp_active_panel_2nd);
			return 0;
		}
	}
	LOG_MSG_DEBUG(ERR_LOG, "[%s] find panel_2nd fail!\n", __func__);
	return -EINVAL;
}
#endif

#if defined(FP_PANEL_NOTIFY_PRIMARY) || defined(FP_PANEL_NOTIFY_SECONDARY)
static int primary_screen_off = 1;
#ifdef FP_PANEL_NOTIFY_SECONDARY
static int secondary_screen_off = 1;
#endif
static void silfp_init_drm_notifier(struct silfp_data *silfp)
{
	int ret = 0;
	int time = 0;
	LOG_MSG_DEBUG(INFO_LOG, "[%s] enter\n", __func__);
...
#ifdef FP_PANEL_NOTIFY_PRIMARY
	ret = silfp_drm_get_pannel(silfp->spi->dev.of_node);
	for (time = 0; time < 25; time++) {
		if (ret) {
			LOG_MSG_DEBUG(ERR_LOG, "[%s] get pannel node error\n", __func__);
			usleep_range(1000000, 1001000);
			ret = silfp_drm_get_pannel(silfp->spi->dev.of_node);
		}
	}
	if (ret) {
		LOG_MSG_DEBUG(ERR_LOG, "[%s] get pannel node error after retrying 10 times\n", __func__);
	}
#endif

#ifdef FP_PANEL_NOTIFY_SECONDARY
	ret = silfp_drm_get_pannel_2nd(silfp->spi->dev.of_node);
	for (time = 0; time < 25; time++) {
		if (ret) {
			LOG_MSG_DEBUG(ERR_LOG, "[%s] get pannel_2nd node error\n", __func__);
			usleep_range(1000000, 1001000);
			ret = silfp_drm_get_pannel_2nd(silfp->spi->dev.of_node);
		}
	}
	if (ret) {
		LOG_MSG_DEBUG(ERR_LOG, "[%s] get pannel_2nd node error after retrying 10 times\n", __func__);
	}
#endif

#ifdef FP_PANEL_NOTIFY_PRIMARY
	cookie = panel_event_notifier_register(PANEL_EVENT_NOTIFICATION_PRIMARY,
		PANEL_EVENT_NOTIFIER_CLIENT_BATTERY_CHARGER, silfp_active_panel,
		&silfp_drm_notifier_callback, silfp);
	if (!cookie) {
		LOG_MSG_DEBUG(ERR_LOG, "[%s] drm_panel_notifier_register error\n", __func__);
		return;
	}
#endif

#ifdef FP_PANEL_NOTIFY_SECONDARY
	/* add PANEL_EVENT_NOTIFIER_CLIENT_FINGERPRINT in panel_event_notifier.h */
	cookie_2nd = panel_event_notifier_register(PANEL_EVENT_NOTIFICATION_SECONDARY,
		PANEL_EVENT_NOTIFIER_CLIENT_FINGERPRINT, silfp_active_panel_2nd,
		&silfp_drm_notifier_callback_2nd, silfp);
	if (!cookie_2nd) {
		LOG_MSG_DEBUG(ERR_LOG, "[%s] drm_panel_notifier_register error\n", __func__);
		return;
	}
...
static void silfp_drm_notifier_callback(enum panel_event_notifier_tag tag,
		struct panel_event_notification *notification, void *client_data)
{
	struct silfp_data *fp_dev = cookie->pvt_data;

	if (!notification) {
		LOG_MSG_DEBUG(ERR_LOG, "[%s] no notification callback", __func__);
		return ;
	}

	LOG_MSG_DEBUG(INFO_LOG, "[%s]: 1st callback", __func__);
	switch (notification->notif_type) {
	case DRM_PANEL_EVENT_UNBLANK: 
		LOG_MSG_DEBUG(INFO_LOG, "[%s] LCD ON\n", __func__);
		fp_dev->scr_off = 0;
		primary_screen_off = 0;
		silfp_netlink_send(fp_dev, SIFP_NETLINK_SCR_ON);
		break;
	case DRM_PANEL_EVENT_BLANK:
		LOG_MSG_DEBUG(INFO_LOG, "[%s] LCD OFF\n", __func__);
#ifdef FP_PANEL_NOTIFY_SECONDARY
		if (secondary_screen_off == 1) {
			fp_dev->scr_off = 1;
			primary_screen_off = 1;
			silfp_netlink_send(fp_dev, SIFP_NETLINK_SCR_OFF);
			LOG_MSG_DEBUG(INFO_LOG, "[%s] turn off\n", __func__);
		} else {
			primary_screen_off = 1;
			LOG_MSG_DEBUG(INFO_LOG, "[%s] keep on\n", __func__);
		}
#else
			fp_dev->scr_off = 1;
			primary_screen_off = 1;
...
static void silfp_drm_notifier_callback_2nd(enum panel_event_notifier_tag tag,
		struct panel_event_notification *notification, void *client_data)
{
	struct silfp_data *fp_dev = cookie_2nd->pvt_data;
	...
	LOG_MSG_DEBUG(INFO_LOG, "[%s]: 2nd callback", __func__);
	switch (notification->notif_type) {
	case DRM_PANEL_EVENT_UNBLANK:
		LOG_MSG_DEBUG(INFO_LOG, "[%s] LCD_2nd ON\n", __func__);
		fp_dev->scr_off = 0;
		secondary_screen_off = 0;
		silfp_netlink_send(fp_dev, SIFP_NETLINK_SCR_ON);
		break;
	case DRM_PANEL_EVENT_BLANK:
		LOG_MSG_DEBUG(INFO_LOG, "[%s] LCD_2nd OFF\n", __func__);
		if (primary_screen_off == 1) {
		    fp_dev->scr_off = 1;
		    secondary_screen_off = 1;
		    silfp_netlink_send(fp_dev, SIFP_NETLINK_SCR_OFF);
		    LOG_MSG_DEBUG(INFO_LOG, "[%s] turn off\n", __func__);
		} else {
		    secondary_screen_off = 1;
		...
		break;
	default:
		LOG_MSG_DEBUG(INFO_LOG, "[%s] Unknown notifier 2nd\n", __func__);
		break;
	}
}
#endif

#define IN_RECOVERY_MODE 4
extern int zte_get_boot_mode(void);
static int silfp_init(struct silfp_data *fp_dev)
{
    int status = 0;
    int is_recovery = zte_get_boot_mode();
	...
	LOG_MSG_DEBUG(INFO_LOG, "[%s] is_recovery=%d\n", __func__, is_recovery);
#if defined(FP_PANEL_NOTIFY_PRIMARY) || defined(FP_PANEL_NOTIFY_SECONDARY)
	LOG_MSG_DEBUG(INFO_LOG, "[%s] FP_PANEL_NOTIFY_PRIMARY\n", __func__);
	if (is_recovery != IN_RECOVERY_MODE) {
		silfp_init_drm_notifier(fp_dev);
	}
#endif
...
#ifdef FP_PANEL_NOTIFY_SECONDARY
	if (cookie_2nd != NULL) {
		panel_event_notifier_unregister(cookie_2nd);
	}
#endif
