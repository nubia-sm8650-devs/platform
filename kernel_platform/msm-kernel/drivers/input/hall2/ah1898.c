	...
	if (ah1898_chip_data->ah1898_enabled == 1) {
		...
		if (value == 1) {
			...
			*/
			...
			/*log for on*/
			pr_info("%s:hall2 ===switch is on!!the value = %d\n", __func__, value);
			/* delete KEY_POWER input report
			input_report_key(ah1898_chip_data->input, KEY_POWER, 1);
			input_sync(ah1898_chip_data->input);
			input_report_key(ah1898_chip_data->input, KEY_POWER, 0);
...
static irqreturn_t ah1898_interrupt(int irq, void *dev_id)
{
	...
	pr_info("%s:chenhui ah1898_2_interrupt!!\n", __func__);
...
static int ah1898_parse_dt(struct platform_device *pdev)
{
	...
	ah1898_chip_data->irq = of_get_named_gpio(pdev->dev.of_node, "ah2,gpio_irq", 0);

	if (!gpio_is_valid(ah1898_chip_data->irq)) {
		pr_info("hall2 gpio irq pin %d is invalid.\n", ah1898_chip_data->irq);
		return -EINVAL;
	}
...
static int set_hall_gpio_state(struct device *dev)
{
	...
	if (IS_ERR_OR_NULL(hall_gpio_pinctrl)) {
		pr_info("Can not get hall_2_gpio_pinctrl\n");
	...
	if (IS_ERR_OR_NULL(hall_gpio_state)) {
		pr_info("Can not get hall_2_gpio_state\n");
	...
	if (error) {
		pr_info("can not set hall_2_gpio pins to zte_hall_gpio_active states\n");
	} else {
		pr_info("set_hall_2_gpio_state success.\n");
	}
	return error;
}
...
static int  ah1898_probe(struct platform_device *pdev)
{
	...
	if (error < 0) {
		pr_info("set_hall_2_gpio_state failed.\n");
	}
	...
	pr_info("hall2 Init hall_state=%d\n", hall_status);
	pr_info("%s:hall2 Init success!\n", __func__);
