...
#include <linux/mman.h>
#endif
#define HAPTIC_HV_DRIVER_VERSION	"v0.5.0.14"

static int irq_disable_cnt = 0;
...
/* Started by AICoder, pid:q56e8t3b10f894714dfc09426013fd8311235ac2 */
void aw_control_irq(struct aw_haptic *aw_haptic, bool en_flag)
{
    if (en_flag) {
        if (aw_haptic->irq_mark == AW_IRQ_ENABLE) {
            aw_info("irq had been enabled!");
            return;
        }
        aw_haptic->irq_mark = AW_IRQ_ENABLE;
        irq_disable_cnt--;
        enable_irq(gpio_to_irq(left->irq_gpio));
    } else {
        if (aw_haptic->irq_mark == AW_IRQ_DISABLE) {
            aw_info("irq had been disabled!");
            return;
        }
        aw_haptic->irq_mark = AW_IRQ_DISABLE;
        irq_disable_cnt++;
        disable_irq(gpio_to_irq(left->irq_gpio));
    }
}

static void haptic_audio_enable_mix_play(struct aw_haptic *aw_haptic, bool en_flag)
{
    uint8_t reg_val = 0;

    if (en_flag) {
        aw_info("config broadcast address");
        right->i2c->addr = right->record_i2c_addr;

        aw_control_irq(left, false);
        left->func->set_pwm(left, AW_PWM_24K);
        right->func->set_pwm(right, AW_PWM_24K);
        i2c_w_bits(left, AW8693X_REG_SYSCTRL5,
        ...
        reg_val = 0xa8;
        i2c_w_bits(left, AW8693X_REG_SYSCTRL2, (~(1 << 7)), (1 << 7));
        /* en PLL */
        i2c_w_bits(right, AW8693X_REG_SYSCTRL4,
                    AW8693X_BIT_SYSCTRL4_EN_DLL_MASK,
                    AW8693X_BIT_SYSCTRL4_EN_DLL_ON);
        i2c_w_bits(right, AW8693X_REG_SYSCTRL5,
                    AW8693X_BIT_SYSCTRL5_EN_BRO_ADDR_MASK,
                    AW8693X_BIT_SYSCTRL5_EN_BRO_ADDR_ON);
        i2c_w_bits(right, AW8693X_REG_SYSCTRL5,
                    AW8693X_BIT_SYSCTRL5_BROADCAST_ADDR_MASK,
                    AW8693X_BROADCAST_ADDR);
        reg_val = 0x68;
        i2c_w_bytes(right, AW8693X_REG_SYSCTRL2, &reg_val, AW_I2C_BYTE_ONE);

        /* change broadcast addr */
        right->i2c->addr = AW8693X_BROADCAST_ADDR;
    } else {
        if (right->i2c->addr != right->record_i2c_addr) {
            /* Close dual rtp */
            right->i2c->addr = AW8693X_BROADCAST_ADDR;
            ...
            /* restore i2c addr */
            right->i2c->addr = right->record_i2c_addr;
            aw_info("restore i2c addr 0x%02x", right->i2c->addr);
            aw_control_irq(left, true);
            left->func->set_pwm(left, AW_PWM_24K);
            right->func->set_pwm(right, AW_PWM_24K);
        } else {
            aw_info("right->i2c->addr had been 0x5B!!!");
        }
    }
}
/* Ended by AICoder, pid:q56e8t3b10f894714dfc09426013fd8311235ac2 */
...
static void tiktap_enable_mix_play(struct aw_haptic *aw_haptic, bool en_flag)
{
	...
	if (en_flag) {
		...
		i2c_w_bits(left, AW8693X_REG_SYSCTRL3,
		...
		i2c_w_bits(right, AW8693X_REG_SYSCTRL3,
							AW8693X_BIT_SYSCTRL3_SLOT_CHSEL_MASK,
		...
		/* change broadcast addr */
		right->i2c->addr = AW8693X_BROADCAST_ADDR;
		i2c_w_bits(right, AW8693X_REG_RTPCFG1,
...
static int get_ram_num(struct aw_haptic *aw_haptic)
{
	...
	aw8693x_broadcast_config(right, false, AW_RTP_MODE);
	/* RAMINIT Enable */
	aw_haptic->func->play_stop(aw_haptic);
...
static bool richtap_rtp_start(struct aw_haptic *aw_haptic)
{
    ...
    aw_haptic->func->play_mode(aw_haptic, AW_RTP_MODE);
    //aw_info("before go addr %x", aw_haptic->i2c->addr);
    aw_haptic->func->play_go(aw_haptic, true);
   ...
   while (cnt) {
    ...
    mutex_unlock(&aw_haptic->lock);
    //aw_info("after go addr %x", aw_haptic->i2c->addr);
...
static void richtap_stop_work(struct work_struct *work)
{
	...
	if (aw_haptic->tiktap_mix_flag && (atomic_read(&aw_haptic->tiktap_dual_rtp) == false)) {
		if (right->i2c->addr == AW8693X_BROADCAST_ADDR) {
...
static void richtap_rtp_work(struct work_struct *work)
{
   ...
   aw_haptic->curr_buf = aw_haptic->start_buf;
    ...
    {
	//aw_info("before write addr %x", aw_haptic->i2c->addr);
...
// 				   break;
//        }
// 		while(!aw_haptic->func->rtp_get_fifo_afi(aw_haptic) && (aw_haptic->curr_buf->status == MMAP_BUF_DATA_VALID))
//        {
...
static long richtap_file_unlocked_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd)
	{
		...
		aw_info("RICHTAP_STREAM_MODE");
			...
			aw_haptic->func->play_stop(aw_haptic);
			if (right->tiktap_mix_flag) {
				tiktap_enable_mix_play(aw_haptic, false);
				right->tiktap_mix_flag = false;
			}
			while (irq_disable_cnt > 0) {
				enable_irq(gpio_to_irq(left->irq_gpio));
				irq_disable_cnt--;
			}
...
#ifndef AW_VIM3
static int irq_config(struct device *dev, struct aw_haptic *aw_haptic)//todo
{
	int ret = -1;
	int irq_flags = 0;
...
static void audio_work_routine(struct work_struct *work)
{
	...
	if (ctr->play == AW_PLAY_ENABLE) {
		...
		if (ctr->cmd & AW_CMD_L_EN) {
			...
			if (ctr->loop == 0xf) {
				left->func->play_mode(left, AW_RAM_LOOP_MODE);
			}
		...
		if (ctr->cmd & AW_CMD_R_EN) {
			...
			if (ctr->loop == 0xf) {
				right->func->play_mode(right, AW_RAM_LOOP_MODE);
			}
		...
		/* adjust by hardware two motor inverse */
		if ((ctr->cmd & AW_CMD_R_EN) && (ctr->cmd & AW_CMD_L_EN)) {
			if (ctr->loop == 0xf) {
				#ifdef DUAL_CURRENT_DIRECTION_OPPOSITE
				left->func->set_wav_seq(left, 0x00, 0x04);//left no.4
				right->func->set_wav_seq(right, 0x00, 0x06);//right no.6
				#else
				left->func->set_wav_seq(left, 0x00, 0x04);//left no.4
				right->func->set_wav_seq(right, 0x00, 0x4);//right no.4
				#endif
			} else if (ctr->loop == 0) {
				#ifdef DUAL_CURRENT_DIRECTION_OPPOSITE
				left->func->set_wav_seq(left, 0x00, 0x01);//left no.1
				right->func->set_wav_seq(right, 0x00, 0x05);//right no.5
				#else
				left->func->set_wav_seq(left, 0x00, 0x01);//left no.1
				right->func->set_wav_seq(right, 0x00, 0x01);//right no.1
				#endif
			}
		}
		...
		/* play go */
		if ((ctr->cmd & AW_CMD_R_EN) && (ctr->cmd & AW_CMD_L_EN)) {
			haptic_audio_enable_mix_play(aw_haptic, true);
			right->func->play_go(right, true);
		} else {
			haptic_audio_enable_mix_play(aw_haptic, false);
			if (ctr->cmd & AW_CMD_L_EN)
...
#ifdef AW_DOUBLE
static ssize_t dual_cali_save_show(struct device *dev, struct device_attribute *attr, char *buf)
{
ssize_t len = 0;
cdev_t *cdev = dev_get_drvdata(dev);
...
len += snprintf(buf + len, PAGE_SIZE - len, "f0_cali_data = 0x%02X\n",
  aw_haptic->f0_cali_store_data);
aw_haptic -> f0_cali_data = aw_haptic->f0_cali_store_data;
return len;
}

static ssize_t dual_cali_save_store(struct device *dev, struct device_attribute *attr,
        const char *buf, size_t count)
{
uint32_t val = 0;
int rc = 0;
...
if (val > 0)
    aw_haptic->f0_cali_store_data = aw_haptic->f0_cali_data ;
...
static DEVICE_ATTR(dual_cali_save,S_IWUSR | S_IRUGO, dual_cali_save_show, dual_cali_save_store);
#endif
...
static void tiktap_work_routine(struct work_struct *work)
{
	...
	while (true) {
		...
		if ((aw_haptic->tiktap_stop_flag == true) ||
			...
			aw_info("length:%02x,status:%02x,data:0:0x%02x,1:0x%02x,10:0x%02x,12:0x%02x",
				tiktap_buf->length,tiktap_buf->status,
				tiktap_buf->data[0],tiktap_buf->data[1],
				tiktap_buf->data[10],tiktap_buf->data[12]);
...
static void haptic_init(struct aw_haptic *aw_haptic)
{
	...
	aw_haptic->rtp_num = sizeof(aw_rtp_name) / AW_RTP_NAME_MAX;
...
static int aw_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
	...
	pr_info("[haptic_hv] aw_i2c_probe enter");
	if (!i2c_check_functionality(i2c->adapter, I2C_FUNC_I2C)) {
		pr_err("[haptic_hv]check_functionality failed");
		return -EIO;
	}
...
#ifndef AW_VIM3
	if (gpio_is_valid(aw_haptic->irq_gpio)) {//todo
...
#else
	if(i2c->addr == 0x5B){//todo
		//irq_flags = irq_get_trigger_type(i2c->irq);
		irq_flags = IRQF_TRIGGER_FALLING | IRQF_ONESHOT;
...
static int __init aw_i2c_init(void)
{
	...
	pr_info("[haptic_hv]aw_haptic driver version %s", HAPTIC_HV_DRIVER_VERSION);

	ret = i2c_add_driver(&aw_i2c_driver);
	if (ret) {
		pr_err("fail to add aw_haptic[haptic_hv] device into i2c");
		return ret;
	}
	return 0;
}
