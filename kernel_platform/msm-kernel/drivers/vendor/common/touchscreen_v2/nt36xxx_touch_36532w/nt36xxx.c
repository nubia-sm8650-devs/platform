 ...
 *
 * $Revision: 138036 $
 * $Date: 2024-04-15 15:44:30 +0800 (週一, 15 四月 2024) $
 *
...
extern void tpd_register_fw_class(struct nvt_ts_data *ts);

#ifdef NVT_USB_DETECT_GLOBAL
extern int32_t nvt_ac_power_plug_set(uint8_t status);
bool NVT_USB_detect_flag;
#endif

extern int zte_set_display_rotation(int mrotation);

static struct tp_point_log point_log[TOUCH_MAX_FINGER_NUM];
static char finger_down[TOUCH_MAX_FINGER_NUM] = {0};
...
#if defined(CONFIG_TOUCHSCREEN_UFP_MAC) && defined(PEN_BTN_EVENT)
#define PEN_BTN_EVENT_SW1_SINGLE_CLICK     1
#define PEN_BTN_EVENT_SW2_SINGLE_CLICK     2
#define PEN_BTN_EVENT_SW1_DOUBLE_CLICK     3
#define PEN_BTN_EVENT_SW2_DOUBLE_CLICK     4
#define PEN_BTN_EVENT_SW1_LONG_PRESS       5
#define PEN_BTN_EVENT_SW2_LONG_PRESS       6

#define PEN_SW1_SINGLE_CLICK     "pen_sw1_single_click=true"
#define PEN_SW2_SINGLE_CLICK     "pen_sw2_single_click=true"
#define PEN_SW1_DOUBLE_CLICK     "pen_sw1_double_click=true"
#define PEN_SW2_DOUBLE_CLICK     "pen_sw2_double_click=true"
#define PEN_SW1_LONG_PRESS       "pen_sw1_long_press=true"
#define PEN_SW2_LONG_PRESS       "pen_sw2_long_press=true"
/*******************************************************
Description:
	Novatek touchscreen pen sw event report function.

return:
	n.a.
*******************************************************/

static inline void report_pen_sw_uevent(char *str)
{
	char *envp[2];

	if (!ufp_tp_ops.uevent_pdev) {
		NVT_ERR("pen sw uevent pdev is null!\n");
		return;
	}
...
/* Started by AICoder, pid:q70cbma8ba86817145ff080c807cb94cec85409f */
void nvt_ts_pen_btn_report(uint8_t pen_btn_id) {
    switch (pen_btn_id) {
        case PEN_BTN_EVENT_SW1_SINGLE_CLICK:
            NVT_LOG("Pen btn event : SW1 single click.\n");
            report_pen_sw_uevent(PEN_SW1_SINGLE_CLICK);
            break;
        case PEN_BTN_EVENT_SW2_SINGLE_CLICK:
            NVT_LOG("Pen btn event : SW2 single click.\n");
            report_pen_sw_uevent(PEN_SW2_SINGLE_CLICK);
            break;
        case PEN_BTN_EVENT_SW1_DOUBLE_CLICK:
            NVT_LOG("Pen btn event : SW1 double click.\n");
            report_pen_sw_uevent(PEN_SW1_DOUBLE_CLICK);
            break;
        case PEN_BTN_EVENT_SW2_DOUBLE_CLICK:
            NVT_LOG("Pen btn event : SW2 double click.\n");
            report_pen_sw_uevent(PEN_SW2_DOUBLE_CLICK);
            break;
        case PEN_BTN_EVENT_SW1_LONG_PRESS:
            NVT_LOG("Pen btn event : SW1 long press.\n");
            report_pen_sw_uevent(PEN_SW1_LONG_PRESS);
            break;
        case PEN_BTN_EVENT_SW2_LONG_PRESS:
            NVT_LOG("Pen btn event : SW2 long press.\n");
            report_pen_sw_uevent(PEN_SW2_LONG_PRESS);
            break;
        default:
            break;
    }
}
/* Ended by AICoder, pid:q70cbma8ba86817145ff080c807cb94cec85409f */
#endif // #if PEN_BTN_EVENT

#if defined(CONFIG_TOUCHSCREEN_UFP_MAC) && defined(PEN_HOUVER_UEVENT)
#define PEN_HOUVER_EVENT     "pen_houver_event=true"
#define PEN_HOUVER_LEAVE_EVENT     "pen_houver_event=false"

static inline void report_pen_houver_uevent(char *str)
{
	char *envp[2];
...
static inline void point_up_log(int index)
{
	if (finger_down[index]) {
		finger_down[index] = 0;
#if defined(CONFIG_TOUCHSCREEN_UFP_MAC) && defined(PEN_HOUVER_UEVENT)
		report_pen_houver_uevent(PEN_HOUVER_LEAVE_EVENT);
		NVT_LOG("report_pen_houver_leave_uevent\n");
#endif
		NVT_LOG("touch_point: touch_up id: %d, coord[%d, %d]\n",
...
static inline void point_down_log(int x, int y, int index)
{
	if (!finger_down[index]) {
		finger_down[index] = 1;
#if defined(CONFIG_TOUCHSCREEN_UFP_MAC) && defined(PEN_HOUVER_UEVENT)
		report_pen_houver_uevent(PEN_HOUVER_EVENT);
		NVT_LOG("report_pen_houver_uevent\n");
#endif
		NVT_LOG("touch_point: touch_down id: %d, coord[%d, %d]\n", index, x, y);
	}
...
/* Started by AICoder, pid:h2972ybaa0da36b148cc0b3210d5c706cc14641a */
#ifdef GTP_REPORT_BY_ZTE_ALGO
	struct ztp_device *cdev = tpd_cdev;
#endif
/* Ended by AICoder, pid:h2972ybaa0da36b148cc0b3210d5c706cc14641a */

#ifdef CONFIG_TOUCHSCREEN_UFP_MAC
#if defined(CONFIG_TOUCHSCREEN_UFP_MAC) && defined(PEN_BTN_EVENT)
	uint32_t pen_btn_event = 0;
#endif
#endif

#if WAKEUP_GESTURE
	if (ts->ztec.is_wakeup_gesture) {
		if (bTouchIsAwake == 0) {
   ...
   if (POINT_DATA_LEN >= POINT_DATA_CHECKSUM_LEN) {
	...
	for (i = 0; i < ts->max_touch_num; i++) {
		...
		if (((point_data[position] & 0x07) == 0x01) || ((point_data[position] & 0x07) == 0x02)) {	//finger down (enter & moving)
			...
			//NVT_ERR("input_x = %d input_y = %d\n", input_x, input_y);

#ifdef GTP_REPORT_BY_ZTE_ALGO
			if (cdev->zte_tp_algo) {
				press_id[input_id - 1] = 1;
				tpd_touch_press(ts->input_dev, input_x, input_y,
					input_id - 1, input_w, 0);
			} else {
#if MT_PROTOCOL_B
				press_id[input_id - 1] = 1;
	...
	for (i = 0; i < ts->max_touch_num; i++) {
		if (press_id[i] != 1) {
#ifdef GTP_REPORT_BY_ZTE_ALGO
			/* Started by AICoder, pid:i0c0e6a1c0x7e2c14f76094a00d57b00ea282c17 */
			if (cdev->zte_tp_algo) {
			...
			/* Ended by AICoder, pid:i0c0e6a1c0x7e2c14f76094a00d57b00ea282c17 */
#else
			input_mt_slot(ts->input_dev, i);
			input_report_abs(ts->input_dev, ABS_MT_TOUCH_MAJOR, 0);
			input_report_abs(ts->input_dev, ABS_MT_PRESSURE, 0);
...
#if defined(CONFIG_TOUCHSCREEN_UFP_MAC) && defined(PEN_BTN_EVENT)
				pen_btn_event = (uint32_t)((point_data[77] >> 2) & 0x07);
#endif // #if PEN_BTN_EVENT
...
#if defined(CONFIG_TOUCHSCREEN_UFP_MAC) && defined(PEN_BTN_EVENT)
				if (pen_btn_event != 0x07) {
					nvt_ts_pen_btn_report(pen_btn_event);
				}
#endif // #if PEN_BTN_EVENT
...
static int nvt_ts_check_dt(struct device_node *np)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (node != NULL)
			NVT_LOG("%s: node = %s", __func__, node->name);
...
static int32_t nvt_ts_probe(struct spi_device *client)
{
	...
	queue_delayed_work(nvt_fwu_wq, &ts->nvt_fwu_work, msecs_to_jiffies(2000));//14000
#endif
	...
	ts->ztec.is_single_tap = 0;
	ts->ztec.is_single_aod = 0;
	ts->ztec.is_single_fp = 0;
	ts->ztec.is_wakeup_gesture = 0;
	ts->ztec.is_one_key = 0;
	ts->ztec.finger_lock_flag = 0;
	ts->ztec.is_set_onekey_in_suspend = 0;
	ts->ztec.is_play_game = 0;
	ts->ztec.is_palm_mode = 0;
	ts->ztec.sensibility_level = 2;
	ts->ztec.follow_hand_level = 2;
	ts->ztec.stability_level = 2;
	ts->ztec.rotation_limit_level = 1;
	ts->ztec.is_fake_sleep = 0;
	ts->ztec.is_fake_sleep_in_suspend = 0;
#ifdef FOR_ZTE_CELL
	ts->ztec.tp_report_rate = 0;
	NVT_LOG("tp_report_rate set 120HZ");
#else
	ts->ztec.tp_report_rate = 1;
	NVT_LOG("tp_report_rate set 240HZ");
#endif

	tpd_register_fw_class(ts);
...
/* Started by AICoder, pid:yfe8cd6e3bud500147400bef405dc40a21183c24 */
#ifdef NVT_USB_DETECT_GLOBAL
    if (NVT_USB_detect_flag) {
        usb_state = 1;
        reteval = nvt_ac_power_plug_set(usb_state);
    }
#endif
/* Ended by AICoder, pid:yfe8cd6e3bud500147400bef405dc40a21183c24 */
	NVT_LOG("ts->ztec.display_rotation = %d, ts->ztec.rotation_limit_level = %d\n", ts->ztec.display_rotation, ts->ztec.rotation_limit_level);
	ret = zte_set_display_rotation(ts->ztec.display_rotation);
	if (ret) {
