...
// zte-modify, modify for FOR_ZTE_CELL, network, begin
#ifdef FOR_ZTE_CELL
#include <cutils/properties.h>
#endif
// zte-modify, modify for FOR_ZTE_CELL, network, end
...
#ifdef FOR_ZTE_CELL
#define ONLY_SHOW_CURR_IFACE
static bool is_other_cell_enable = false;
static bool is_other_cell_init = false;
static char findStrCache[300];
#ifdef ONLY_SHOW_CURR_IFACE
//only show fg cell used ifaces
static bool is_cell_skip_iface(const char *name) {
    if (!is_other_cell_enable){
        return false;
    }
    char cell_name[PROPERTY_VALUE_MAX];
    property_get("ro.cell.name", cell_name, "host");
    bool isHome = (strcmp(cell_name, "home") == 0);
    bool isWork = (strcmp(cell_name, "work") == 0);
    if ((!isHome) && (!isWork) ){
        return false;
    }
    ...
    bool res = true;
    if ((isHome || isWork)
        && ((strncmp(name, "rmne", 4) == 0)
            || (strncmp(name, "wlan", 4)== 0)
            || (strncmp(name, "tun", 3)== 0))) {
        //only use curr prop
        if (true){
            char allIfaces[200];
            property_get("sys.dualsys.ifaces", allIfaces, "if:");
            char findStrCurr[50];
            strcpy(findStrCurr, name);
            strcat(findStrCurr, ";");
            if(strstr(allIfaces, findStrCurr)){
                return false;
            }else{
                return true;
            }
        }
        if (!is_other_cell_init){
            memset(findStrCache, 0, sizeof(findStrCache));
            FILE* file = fopen("/proc/net/netisolation/ni_ifs", "r");
            if (file != NULL) {
                int row = -1;
                char line[100];
                char findStr[50];
                strcat(findStr, isWork ? (" --> work") : (" --> home"));
                while (fgets(line, sizeof(line), file) != NULL){
                    ...
                    if (row > 15 ){
                        ...
                        if (strstr(line, findStr)){
                            strcat(findStrCache, line);
                        }
                    }
                }
                fclose(file);
            }
            is_other_cell_init = true;
        }
        char findStrCurr[50];
        strcpy(findStrCurr, name);
        strcat(findStrCurr, isWork ? (" --> work") : (" --> home"));
        if(strstr(findStrCache, findStrCurr)){
            return false;
        }
    }
    return res;
    ...
    if ((isHome || isWork)
        ...
        if (!is_other_cell_init){
            ...
            if (file != NULL) {
                ...
                char line[100];
                char findStr[50];
                strcat(findStr, isHome ? (" --> work") : (" --> home"));
                while (fgets(line, sizeof(line), file) != NULL){
                    row ++;
                    if (row > 15 ){
                        break;
        ...
        char findStrCurr[50];
        strcpy(findStrCurr, name);
        strcat(findStrCurr, isHome ? (" --> work") : (" --> home"));
        if(strstr(findStrCache, findStrCurr)){
            return true;
        }
    }
		   ...
		   struct nlmsghdr *n, void *arg)
{
    ...
    #ifdef FOR_ZTE_CELL
    if (tb[IFLA_IFNAME]) {
        if (is_cell_skip_iface(rta_getattr_str(tb[IFLA_IFNAME]))){
            return -1;
        }
    }
    #endif
...
static int ipaddr_list_flush_or_save(int argc, char **argv, int action)
{
    ...
    // zte-modify, modify for FOR_ZTE_CELL, network, begin
    #ifdef FOR_ZTE_CELL
    is_other_cell_enable = (action == IPADD_LIST) ? true : false;
    is_other_cell_init = false;
    #endif
    // zte-modify, modify for FOR_ZTE_CELL, network, end
