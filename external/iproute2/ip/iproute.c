...
// zte-modify, modify for FOR_ZTE_CELL, network, begin
#ifdef FOR_ZTE_CELL
#define ONLY_SHOW_CURR_IFACES
#ifdef ONLY_SHOW_CURR_IFACES
static bool is_cell_skip_ifaces(const char *name) {
...
int print_route(const struct sockaddr_nl *who, struct nlmsghdr *n, void *arg)
{
    ...
    // zte-modify, modify for FOR_ZTE_CELL, network, begin
    #ifdef FOR_ZTE_CELL
    if (((table== RT_TABLE_LOCAL) || (table== RT_TABLE_MAIN)) && tb[RTA_OIF]){
        if (is_cell_skip_ifaces(ll_index_to_name(rta_getattr_u32(tb[RTA_OIF])))){
            return 0;
        }
    }
    #endif
