...
int32_t RamWrite16A_ois_dw9784(void *data, int addr, int value)
{
...
int32_t RamRead16A_ois_dw9784(void *data, int addr, uint32_t *value)
{
	msm_ois_debug_info_t *ptr = (msm_ois_debug_info_t *)data;
	int32_t rc = 0;
...
int dw9784_wait_check_register(void *data, int addr, uint32_t value)
{
	/*
	...
	*/
	...
	for (i = 0; i < LOOP_A; i++) {
		RamRead16A_ois_dw9784(data, addr, &r_data);  //  Read status
		if (r_data == value) {
			break;
		}
		else {
			if (i >= LOOP_B) {
				pr_err("%s:%d: [dw9784_wait_check_register]fail: 0x%04X",
...
void dw9784_code_pt_off(void *data)
{
	pr_info("%s:%d: [dw9784_code_pt_off] start", __func__, __LINE__);
	/* release all protection */
	RamWrite16A_ois_dw9784(data, 0xFD00, 0x5252);
	mdelay(1);
	pr_info("%s:%d: [dw9784_code_pt_off] finish", __func__, __LINE__);
}

void ois_reset_ois_dw9784(void *data)
{
	pr_info("%s:%d: [dw9784_ois_reset_ois_dw9784] ois reset", __func__, __LINE__);

	RamWrite16A_ois_dw9784(data, 0xD002, 0x0001);  /* printfc reset */
	mdelay(4);
	RamWrite16A_ois_dw9784(data, 0xD001, 0x0001);  /* Active mode (DSP ON) */
	mdelay(25);  /* ST gyro - over wait 25ms, default Servo On */
	RamWrite16A_ois_dw9784(data, 0xEBF1, 0x56FA);  /* User protection release */
}
...
int calibration_save_ois_dw9784(void *data)
{
	...
	mdelay(1);
	RamWrite16A_ois_dw9784(data, 0x7010, 0xA000);  // Set store mode standby
	mdelay(1);
	dw9784_code_pt_off(data); /* code protection off */
	RamWrite16A_ois_dw9784(data, 0x700F, 0x5959);  // Set protect code
	mdelay(1);
	RamWrite16A_ois_dw9784(data, 0x7011, 0x0001);  // Execute store
	mdelay(40);

	//  When store is done, status changes to 0xA001
	if (dw9784_wait_check_register(data, 0x7010, 0xA001) == FUNC_PASS) {
		ois_reset_ois_dw9784(data);
		pr_info("%s:%d: [dw9784_set_cal_store] finish", __func__, __LINE__);
	}
	else {
		pr_err("%s:%d: [dw9784_set_cal_store] store function fail", __func__, __LINE__);
		return FUNC_FAIL;
	}
	return FUNC_PASS;
}

int ois_dw9784_debugfs_cal_s(void *data)
{
	msm_ois_debug_info_t *ptr = (msm_ois_debug_info_t *) data;
	uint32_t status;
	...
	pr_info("%s:%d:[dw9784_gyro_offset_calibrtion] gyro_offset_calibrtion starting", __func__, __LINE__);

	ptr->cal_info.cal_success = 0;
	RamWrite16A_ois_dw9784(data, 0x7012, 0x0006);  // gyro offset calibration
	mdelay(1);
	RamWrite16A_ois_dw9784(data, 0x7010, 0x6000);  
	mdelay(1);

	if (dw9784_wait_check_register(data, 0x7010, 0x6000) == FUNC_PASS) {
		RamWrite16A_ois_dw9784(data, 0x7011, 0x0001);		// gyro ofs calibration execute command
		mdelay(1);
	}
	else {
		pr_err("[dw9784_gyro_ofs_calibration] FUNC_FAIL");
		return FUNC_FAIL;
	}

	//  when calibration is done, Status changes to 0x6001
	if (dw9784_wait_check_register(data, 0x7010, 0x6001) == FUNC_PASS) {
		pr_info("[dw9784_gyro_ofs_calibration]calibration function finish");
	}
	else {
		pr_err("[dw9784_gyro_ofs_calibration]calibration function error");
		return FUNC_FAIL;
	}
	...
	if ((status & 0x8000)== 0x8000) {
		...
		else
		{
			...
			pr_err("[dw9784_gyro_ofs_calibration] x gyro ofs cal fail");
		}

		if ((status & 0x10) == X_AXIS_GYRO_OFS_OVER_MAX_LIMIT) {
			msg += X_AXIS_GYRO_OFS_OVER_MAX_LIMIT;
			pr_info("[dw9784_gyro_ofs_calibration] x gyro ofs over the max. limit");
		}

		if ((status & 0x2) == Y_AXIS_GYRO_OFS_PASS) {
			msg += EOK;
			pr_info("[dw9784_gyro_ofs_calibration] y gyro ofs cal pass");
		}else
		{
			msg += Y_AXIS_GYRO_OFS_FAIL;
			pr_err("[dw9784_gyro_ofs_calibration] y gyro ofs cal fail");
		}

		if ((status & 0x20) == Y_AXIS_GYRO_OFS_OVER_MAX_LIMIT) {
			msg += Y_AXIS_GYRO_OFS_OVER_MAX_LIMIT;
			pr_info("[dw9784_gyro_ofs_calibration] y gyro ofs over the max. limit");
		}

		if ((status & 0x800) == XY_AXIS_CHECK_GYRO_RAW_DATA) {
			msg += XY_AXIS_CHECK_GYRO_RAW_DATA;
			pr_info("[dw9784_gyro_ofs_calibration] check the x/y gyro raw data");
		}
		pr_info("[dw9784_gyro_ofs_calibration] x/y gyro ofs calibration finish");
