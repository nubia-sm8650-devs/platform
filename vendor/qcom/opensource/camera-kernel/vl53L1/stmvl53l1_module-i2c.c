...
static int stmvl53l1_parse_tree(struct device *dev, struct i2c_data *i2c_data)
{
	...
	if (force_device) {
		...
		if (IS_ERR(i2c_data->vdd) || i2c_data->vdd == NULL) {
			i2c_data->vdd = NULL;
			/* try gpio */
			/*rc = of_property_read_u32_array(dev->of_node,"pwren-gpio", &i2c_data->pwren_gpio, 1);*/
			i2c_data->pwren_gpio = of_get_named_gpio_flags(dev->of_node, "pwren-gpio", 0, NULL);
		...
		rc = of_property_read_u32_array(dev->of_node, "reg",
			&reg, 1);
		if (rc) {
			vl53l1_wanrmsg("Unable to find reg %d 0x%x",
				rc, i2c_addr_nb);
			reg = i2c_addr_nb;
		}
		client->addr = reg;
...
void stmvl53l1_exit_i2c(void *i2c_object)
{
	vl53l1_dbgmsg("Enter\n");
	i2c_del_driver(&stmvl53l1_driver);
	vl53l1_dbgmsg("End\n");
