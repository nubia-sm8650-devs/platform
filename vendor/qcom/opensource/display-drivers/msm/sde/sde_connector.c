...
#include "sde_trace.h"
#include "../zte_disp/zte_panel_feature.h"
...
int sde_connector_update_hbm(struct drm_connector *connector, struct sde_encoder_virt *sde_enc)
{
	...
	if (ismask != panel->is_hbm_enabled) {
		...
		SDE_INFO("MSM_LCD distance from TE is %lld us in fps %d\n", diff, panel->cur_mode->timing.refresh_rate);
		if ( panel->cur_mode->timing.refresh_rate < 120 && panel->vsync_width) {
			us_per_frame = 1000000 / panel->cur_mode->timing.refresh_rate;
			diff = diff % us_per_frame;
			if( diff < panel->vsync_width) {
				delay = panel->vsync_width - diff;
			} else {
				rest = us_per_frame - diff;
				delay = rest < REST_TIME ? us_per_frame - diff + panel->vsync_width : 0;
			}
			if (delay > 0) {
				SDE_INFO("MSM_LCD code sent delay  %lld us", delay);
				usleep_range(delay, delay + 100);
			}
		}
		zte_set_disp_parameter(panel, ZTE_LCD_HBM_CTRL, ismask, false);
	...
	if (isfod != panel->fod_layer) {
		if (isfod) {
			if (ismask) {
				...
				if (panel->in_aod) {
					queue_delayed_work(panel->fod_uiready_wq, &panel->fod_uiready_delayed_work, msecs_to_jiffies(50));
				} else {
					queue_delayed_work(panel->fod_uiready_wq, &panel->fod_uiready_delayed_work, 0);
				}
			}
		} else {
			panel->fod_layer = isfod;
	...
	if (isaod != panel->aod_layer) {
		...
		aod_event = isaod ? AOD_DATA_APPEAR : AOD_DATA_DISAPPEAR;
		zte_aod_event_work_handler(panel, aod_event);
	}
	return 0;
}
// #endif
