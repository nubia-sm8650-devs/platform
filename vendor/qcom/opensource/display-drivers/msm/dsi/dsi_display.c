		...
		int power_mode, void *disp)
{
	...
	case SDE_MODE_DPMS_LP1:
		...
		display->panel->disp_feature->zte_panel_state = SDE_MODE_DPMS_LP1;
	...
	case SDE_MODE_DPMS_LP2:
		...
		if (!atomic_read(&display->panel->pm_aod))
			atomic_inc(&display->panel->pm_aod);
		display->panel->disp_feature->zte_panel_state = SDE_MODE_DPMS_LP2;
		break;
	case SDE_MODE_DPMS_ON:
		...
		display->panel->disp_feature->zte_panel_state = SDE_MODE_DPMS_ON;
	...
	for (mode_idx = 0; mode_idx < timing_mode_count; mode_idx++) {
		...
		/*if(display->panel->host_config.dst_format == DSI_PIXEL_FORMAT_RGB101010 &&
			display_mode.timing.dsc_enabled) {
				...
				DSI_INFO("MSM_LCD change pixel %d to %d", DSI_PIXEL_FORMAT_RGB101010, DSI_PIXEL_FORMAT_RGB888);
		}*/
			 ...
			 u32 flags)
{
	...
	DSI_INFO("msm_lcd mdp_transfer_time=%d, hactive=%d, vactive=%d, fps=%d, clk_rate=%llu\n",
